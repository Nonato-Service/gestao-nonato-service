(function (global) {
  ...
  const DB_VERSION = 2;
  const STORES = ["settings", "servicoGrupos", "servicos", "relatorios", "despesasDocs", "cartoes", "campoUsers"];
  ...
  async function getCampoUsers() {
    return getJson("campoUsers", "list", []);
  }

  async function saveCampoUsers(list) {
    await setJson("campoUsers", "list", Array.isArray(list) ? list : []);
  }
