(function () {
  ...
  const Auth = window.NCampoAuth;
  const Gestao = window.NCampoGestao;
  ...
  function updateTopBrand() {
    ...
    if (user && chip) {
      chip.hidden = false;
      chip.textContent = user.name || user.login;
    } else if (chip) chip.hidden = true;
    if (logoutBtn) logoutBtn.hidden = !user;

    const sub = $("topStatus");
    if (sub) {
      if (!navigator.onLine) sub.textContent = t("statusOffline");
      else if (user) sub.textContent = t("statusLoggedAs", { name: user.name || user.login });
      else sub.textContent = t("appSub");
    }
  }
  ...
  function applyNavPermissions() {
    ...
    document.querySelectorAll(".tabs__btn[data-view]").forEach((btn) => {
      ...
      const perm = permMap[view] || view;
      btn.hidden = !Auth.can(perm);
    });
  }
  ...
  function showApp() {
    ...
    I18n.applyStaticUi();
    updateTopBrand();
    const allowed = Auth.firstAllowedView();
    if (!Auth.can(state.view === "logo" ? "empresa" : state.view)) state.view = allowed;
    render();
  }

  function showLogin(showBootstrap) {
    $("app").hidden = true;
    $("loginScreen").hidden = false;
    $("bootstrapBox").hidden = !showBootstrap;
    $("loginSub").textContent = t("loginSub");
    $("loginUserLabel").textContent = t("loginUserLabel");
    $("loginPassLabel").textContent = t("loginPassLabel");
    $("btnLogin").textContent = t("loginBtn");
    $("loginHint").textContent = t("loginHint");
    $("bootstrapTitle").textContent = t("bootstrapTitle");
    $("bootstrapHint").textContent = t("bootstrapHint");
    $("bootNameLabel").textContent = t("bootstrapNameLabel");
    $("btnBootstrap").textContent = t("bootstrapBtn");
  }
  ...
  async function tryLogin() {
    ...
    try {
      ...
      try {
        await Auth.loginLocal(login, password, Db);
        showApp();
        return;
      } catch {
  ...
  function setView(view) {
    ...
    const perm = permMap[view];
    if (perm && !Auth.can(perm)) {
      state.view = Auth.firstAllowedView();
    } else {
      state.view = view;
    }
    ...
    main.innerHTML = `
      ...
      <div class="panel">
        ...
        <div class="list__actions" style="margin-top:12px">
          ${Auth.can("relatorios") ? `<button type="button" class="btn btn--primary" data-go="relatorios">${t("newReport")}</button>` : ""}
          ${Auth.can("despesas") ? `<button type="button" class="btn" data-go="despesas">${t("registerExpenses")}</button>` : ""}
          ${Auth.can("empresa") ? `<button type="button" class="btn" data-go="logo">${t("logoAndCompany")}</button>` : ""}
        </div>
      </div>`;
  ...
  function render() {
    ...
    else if (state.view === "gestao") Gestao.renderGestao(main, render);
    else renderHome();
  }
  ...
  $("btnBootstrap")?.addEventListener("click", async () => {
    const login = $("loginUser").value.trim() || "gestor";
    const password = $("loginPass").value;
    const name = $("bootName").value.trim() || "Gestor";
    if (password.length < 4) {
      alert(t("managePasswordShort"));
      return;
    }
    try {
      await Auth.bootstrapLocalGestor(login, password, name, Db);
      showApp();
    } catch (err) {
      alert(err.message || t("loginFailed"));
    }
  ...
  load().then(async () => {
    ...
    document.querySelectorAll(".lang-btn[data-lang]").forEach((btn) => {
      ...
      btn.setAttribute("aria-pressed", loc === I18n.getLocale() ? "true" : "false");
