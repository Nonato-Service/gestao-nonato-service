  ...
  useLayoutEffect(() => {
    ...
    void runPecasBibliotecaSyncExclusive('urgent-loader', async () => {
      try {
        ...
        // Fundir com local — senão categorias já atribuídas neste telemóvel desaparecem.
        const localBefore = Array.isArray(local) ? local : []
        const merged = mergePecasBibliotecaArrays(fromLite, localBefore) as PecaLike[]
        await savePecasBibliotecaLocally(merged)
        void pushPecasBibliotecaClassificationsIfRicher(merged)
        onLoaded(merged)
        onProgress?.('')
        console.info(`[Nonato] Urgent loader: ${merged.length} peça(s) (categorias locais preservadas).`)
        return merged
      } catch (e) {
