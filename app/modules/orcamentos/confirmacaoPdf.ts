/** PDF de confirmação / pedido de orçamento avulso — HTML puro. Relógio injectado. */
...
import {
  ...
  escapePdfHtml,
  wrapPdfTableHtml,
} from '../pdf/documentShell'
import { buildPdfDocumentFooterHtml } from '../pdf/documentLayout'
import { normalizePdfModelo } from '../pdf/modelos'
...
export type BuildOrcamentoConfirmacaoPdfHtmlOpts = {
...
export function buildOrcamentoConfirmacaoPdfHtml(
  options: BuildOrcamentoConfirmacaoPdfHtmlOpts,
  nowMs: number
): string {
  ...
  const dataGeracao = new Date(nowMs).toLocaleString(lang.startsWith('pt') ? 'pt-PT' : lang)
  const docRef = String(orcamento.numeroOrcamento || '').trim() || String(nowMs).slice(-6)
