/** Situações de logo PDF (Administrador) — catálogo e acentos, sem I/O. */
...
export const PDF_LOGO_SITUATION_ACCENT: Record<PdfLogoSituationId, string> = {
  relatorios: 'green',
  fechamentos: 'amber',
  orcamentoPecas: 'blue',
...
export function resolvePdfLogoSituation(id: PdfLogoSituationId): PdfLogoSituationDef {
  return PDF_LOGO_SITUATIONS.find((s) => s.id === id) || PDF_LOGO_SITUATIONS[0]
}

export function pdfLogoSituationAccent(id: PdfLogoSituationId): string {
  return PDF_LOGO_SITUATION_ACCENT[id] ?? 'green'
}
