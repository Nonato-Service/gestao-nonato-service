...
export type HomagExportIdentity = {
  id: string
  dataCriacao: string
}
...
export function homagItemToPecaMerge(
  item: Record<string, unknown>,
  seq: number,
  identity: HomagExportIdentity
): PecaHomagMerge {
  ...
  return {
    ...
    imagem,
    dataCriacao: identity.dataCriacao,
  }
}
...
export function mergeHomagExportIntoBiblioteca<T extends PecaHomagMerge>(
  ...
  makeIdentity: (seq: number) => HomagExportIdentity
  ...
  incomingRaw.forEach((item, idx) => {
    const incoming = homagItemToPecaMerge(item, idx, makeIdentity(idx)) as T
