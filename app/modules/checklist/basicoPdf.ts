/** PDF / envio do checklist básico — HTML e texto puros. Sem window/I/O. */

import type { ChecklistBasicoInstancia } from './basicoTipos'

export type ChecklistBasicoPdfLabels = Record<string, string>

export function escapeChecklistBasicoHtml(s: string): string {
  return String(s ?? '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
export function telefoneDigitsParaWa(telefones: string): string {
