/** Merge unificado manuais ↔ Bíblia (família → marca → modelo). Sem I/O. */
...
export type ConhecimentoMergeIdFactory = (prefix: 'ctg' | 'ctm') => string
...
export function mergeManuaisPayloads(
  server: ManuaisFamiliasGruposPayload,
  local: ManuaisFamiliasGruposPayload
...
function findModeloInGrupo(
  modelos: ManuaisModelo[],
  grupoId: string,
  nome: string,
  bibliaId?: string
): ManuaisModelo | undefined {
  if (bibliaId) {
...
export function mergeBibliaIntoManuais(
  ...
  for (const fam of biblia.familias) {
    ...
    for (const lin of fam.linhas) {
      ...
      const { grupos: gNext, grupoId } = ensureGrupo(grupos, familiaKey, marcaNome, makeId)
      grupos = gNext
...
export function manuaisToBibliaStore(
  ...
  const familias = (payload.familias || [])
    .map((famNome, fi) => {
      ...
      return {
        id: famGrupos[0]?.id ? `fam-${normName(famNome)}` : makeFamiliaId(),
        nome: famNome,
        ordem: fi,
        linhas,
      }
