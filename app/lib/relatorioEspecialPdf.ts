...
export function imprimirRelatorioEspecialPdf(
  ...
  const theadEquipCols = equipamentos
    .map((eq, i) => `<th colspan="3">${escapeHtml(labelEquipamento(eq, i))}</th>`)
    .join('')
  ...
  const subheadEquip = equipamentos
    .map(
      () =>
        `<th>${escapeHtml(L(labels, 'inicio', 'Início'))}</th><th>${escapeHtml(L(labels, 'fim', 'Fim'))}</th><th>${escapeHtml(L(labels, 'totalLiquido', L(labels, 'total', 'Total')))}</th>`
    )
    .join('')
...
<div class="pdf-doc">
  ...
  <h3>${escapeHtml(L(labels, 'relatorioEspecialEquipamentos', 'Equipamentos'))} (${equipamentos.length})</h3>
  <table class="re-especial-table">
    <thead><tr><th>#</th><th>ID</th><th>${escapeHtml(L(labels, 'modelo', 'Modelo'))}</th><th>S/N</th></tr></thead>
    <tbody>
      ${equipamentos
        .map(
          (eq, i) =>
  ...
  <h3>${escapeHtml(L(labels, 'relatorioEspecialPdfControloHoras', 'Controlo de horas por equipamento'))}</h3>
  <table class="re-especial-table">
    <thead>
      <tr>
        <th rowspan="2">${escapeHtml(L(labels, 'data', 'Data'))}</th>
        <th colspan="3">${escapeHtml(L(labels, 'relatorioEspecialPdfIda', 'Ida'))}</th>
        <th colspan="3">${escapeHtml(L(labels, 'relatorioEspecialPdfRetorno', 'Retorno'))}</th>
        <th rowspan="2">KM</th>
        ${theadEquipCols}
        <th rowspan="2">${escapeHtml(L(labels, 'relatorioEspecialPdfDescricao', 'Descrição'))}</th>
      </tr>
      <tr>
        <th>${escapeHtml(L(labels, 'saida', 'Saída'))}</th><th>${escapeHtml(L(labels, 'chegada', 'Chegada'))}</th><th>${escapeHtml(L(labels, 'duracao', 'Duração'))}</th>
        <th>${escapeHtml(L(labels, 'saida', 'Saída'))}</th><th>${escapeHtml(L(labels, 'chegada', 'Chegada'))}</th><th>${escapeHtml(L(labels, 'duracao', 'Duração'))}</th>
        ${subheadEquip}
      </tr>
    </thead>
    <tbody>
      ${linhasDias || `<tr><td colspan="20">${escapeHtml(L(labels, 'relatorioEspecialPdfSemDias', 'Sem dias registados'))}</td></tr>`}
      ${totaisEquip}
      <tr style="background:#e8f5e9;font-weight:700">
        <td colspan="${8 + equipamentos.length * 3}">${escapeHtml(L(labels, 'relatorioEspecialPdfTotalGeralLabel', 'TOTAL GERAL DE HORAS DE TRABALHO'))}</td>
